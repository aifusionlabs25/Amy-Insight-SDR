...
export default function InteractiveAvatar({ userName, userEmail }: InteractiveAvatarProps) {
    ...
    // Global Event Listener for Search Trigger (Nuclear Option)
    useEffect(() => {
        (window as any).openSearchPanel = () => {
            console.log('[InteractiveAvatar] 🔓 Force-opening Search Panel via Global Event');
            setIsSearchOpen(true);
        };
        return () => { delete (window as any).openSearchPanel; };
    }, []);
